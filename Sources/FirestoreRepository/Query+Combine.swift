...
extension Query: CollectionPublishable {
    ...
    public func publisher<Document>() -> AnyPublisher<[Document]?, Error> where Document : Decodable {
        publisher(includeMetadataChanges: true)
    }

    public func publisher<Document>(includeMetadataChanges: Bool = true) -> AnyPublisher<[Document]?, Error> where Document : Decodable {
